...
async def open_all(message: discord.Message, argument: str):
    ...
    if not global_vars.gamemaster_role in global_vars.server.get_member(message.author.id).roles:
        await message_utils.safe_send(message.author, "You don't have permission to open PMs and nominations.")
        return
...
async def close_all(message: discord.Message, argument: str):
    ...
    if not global_vars.gamemaster_role in global_vars.server.get_member(message.author.id).roles:
        await message_utils.safe_send(message.author, "You don't have permission to close PMs and nominations.")
        return
...
async def whisper_mode(message: discord.Message, argument: str):
    ...
    if not global_vars.gamemaster_role in global_vars.server.get_member(message.author.id).roles:
        await message_utils.safe_send(message.author, "You don't have permission to change the whispermode.")
        return
    ...
    if (new_mode):
        ...
        for memb in global_vars.gamemaster_role.members:
            await message_utils.safe_send(memb, "{} has set whisper mode to {}.".format(message.author.display_name,
